...
/**
 * שמירה לכל מהלך — מיד, בלי השהיה.
 *
 * ניסיתי כאן קודם השהיה קצרה שתאחד לחיצות קרובות. זו הייתה טעות:
 * השהיה של חצי שנייה נשמעת כמו כלום, אבל אם הדפדפן נסגר בתוכה —
 * המהלך שנעשה נעלם, ומבחינת מי שמשחק זה "השמירה לא עובדת". בדיקת
 * הקריסה תפסה בדיוק את זה.
 *
 * ולמה זה בסדר לכתוב בכל לחיצה? כי הכתיבה **חוסמת** את הדפדפן.
 * כל עוד היא רצה אי אפשר ללחוץ שוב, ולכן "רצף לחיצות מהיר" הוא
 * תרחיש של סקריפט ולא של אצבע. מה שכן נחסך: אם שום דבר לא זז
 * מאז השמירה האחרונה — לא כותבים בכלל.
 */
function saveSoon() {
  if (!game) return;
  if (stateStamp() === lastStamp) return;      // כלום לא השתנה
  saveGame();
}

// כל כמה זמן בודקים אם המצב זז בלי שנשמר. 8 שניות הוא פשרה בין
// "לא לאבד מהלך" לבין לא לדחוס קריירה שלמה סתם.
const HEARTBEAT_MS = 8000;

/**
 * טביעת אצבע זולה של המצב.
 *
 * רשת ביטחון: אם מטפל כלשהו ישנה משהו וישכח לקרוא ל-saveSoon,
 * פעימת הלב תבחין שהמצב זז ותשמור. היא לא מושלמת — היא לא סורקת
 * את כל הקריירה — אבל היא מכסה כל דבר שהמשתמש יכול לשנות ממסך,
 * והיא עולה מיקרו-שניות ולכן אפשר להריץ אותה כל כמה שניות.
 */
function stateStamp() {
  if (!game) return "";
  const me = game.me;
  const t = game.tactics || {};
  return [game.year, game.week, game.stage, game.money, game.trainingFocus,
          game.intensity, t.formation, t.mentality, t.pressing,
          me.number, me.role, me.duty, Math.round(me.morale), overall(me),
          game.honours.length, game.news.length,
          Object.keys(game.flags).length,
          JSON.stringify(game.flags.agent || null).length,
          JSON.stringify(game.flags.partner || null).length].join("|");
}
...
function refreshSaveBadge() {
  // querySelectorAll ולא querySelector: הסמן מופיע גם בסרגל העליון
  // וגם במסך השמירה, וקודם רק הראשון שבהם התעדכן.
  document.querySelectorAll("[data-save-badge]").forEach(badge => {
    const dot = badge.classList.contains("save-dot");
    badge.textContent = dot ? (saveState.ok ? "✓" : "✕")
                            : (saveState.ok ? savedAgo() : "לא נשמר");
    badge.classList.toggle("bad", !saveState.ok);
    if (!dot || !saveState.ok) return;
    // הבהוב קצר: מי שמשחק צריך *לראות* שנשמר, אחרת הוא מחפש כפתור
    badge.classList.remove("blip");
    void badge.offsetWidth;
    badge.classList.add("blip");
  });
}
...
/**
 * שורת המצב במסך הראשי.
 *
 * קודם ישב כאן פאנל שלם עם כפתורי קבצים, והוא נקרא כמו מטלה: מי
 * שמשחק הבין שהוא צריך "לעצור ולשמור". השמירה אוטומטית לגמרי, ולכן
 * מה שנשאר כאן הוא משפט אחד — וכל השאר עבר למסך נפרד.
 */
/** מסך השמירה: מה שהיה פאנל במסך הראשי, עכשיו במקום משלו. */
function screenSave() {
  return `<div class="screen">
    ${savePanel()}
    <div class="panel">
      <div class="panel-head"><span class="t">איך זה עובד</span></div>
      <div class="panel-body">
        <div class="notes">
          <div class="note"><span class="ico">✓</span><span>כל מהלך נשמר
            ברגע שעשית אותו — שבוע, החלטה, בחירת אימון, טקטיקה.</span></div>
          <div class="note"><span class="ico">✓</span><span>גם סגירה
            פתאומית של הדפדפן לא מוחקת כלום. נבדק בקריסה אמיתית.</span></div>
          <div class="note"><span class="ico">✓</span><span>אין כפתור
            "שמור" ואין צורך בו. הכפתורים כאן הם לגיבוי בלבד.</span></div>
        </div>
      </div>
    </div>
    <button class="btn ghost wide" data-go="main">חזרה</button>
  </div>`;
}
...
function saveLine() {
  ...
  return `
  <div class="panel wide save-line ${ok ? "" : "warn"}">
    <div class="row link" data-go="save">
      <span class="msg-ico">${ok ? "✓" : "⚠️"}</span>
      <span class="grow"><span class="nm">${ok
        ? "הקריירה נשמרת מעצמה, אחרי כל מהלך"
        : "השמירה נכשלה"}</span>
        <span class="sub">${ok
          ? esc(savedAgo() || "נשמר עכשיו") + " · אין צורך לשמור ידנית"
      ...
      <div class="row">
        <span class="grow"><span class="nm">${
          ...
          <span class="sub">${saveState.ok
            ? "אחרי כל מהלך — שבוע, החלטה, בחירת אימון, טקטיקה. בלי לגעת בכלום."
            : esc(saveState.error)}</span></span>
...
function ageBlurb(age, role) {
  ...
  if (age <= 14) return "הכי מוקדם שאפשר. בית ספר, מגרש שכונתי, ומאמן שעוד לא יודע איך קוראים לך — אבל גם השנים שבהן צופי אקדמיות מסתובבים בטורנירי נוער. אם כמה מהן יריבו עליך, הערך שלך יקפוץ עוד לפני שהוכחת משהו.";
  if (age <= 15) return "עדיין בקבוצת הנוער, ועדיין בטווח שבו אקדמיות פונות למשפחה. הדרך ארוכה — והתקרה הכי גבוהה.";
...
function appbar() {
  ...
  <header class="appbar">
    <div class="appbar-top">
      ...
      <span class="save-dot ${saveState.ok ? "" : "bad"}" data-save-badge
        title="${saveState.ok ? "נשמר אוטומטית" : "השמירה נכשלה"}"
        >${saveState.ok ? "✓" : "✕"}</span>
      <div class="date">
...
function screenMain() {
  ...
  if (view === "save") return appbar() + screenSave();
  ...
  <div class="screen">
    ...
    <div class="panel">
      ...
      <div class="panel-body">
        ...
        <div class="chips">
          ${Object.entries(D.PRESSING).map(([k, v]) => `<button class="chip" data-pressing="${k}"
...
function bind() {
  ...
  app.querySelectorAll("[data-focus]").forEach(el =>
    el.addEventListener("click", () => {
      game.setAction(el.dataset.focus);
      saveSoon();
      render();
    }));
  app.querySelectorAll("[data-int]").forEach(el =>
    el.addEventListener("click", () => {
      game.intensity = +el.dataset.int;
      saveSoon();
      render();
    }));
  ...
  app.querySelectorAll("[data-form]").forEach(el =>
    el.addEventListener("click", () => {
      game.tactics.formation = el.dataset.form;
      saveSoon();
      render();
    }));
  app.querySelectorAll("[data-ment]").forEach(el =>
    el.addEventListener("click", () => {
      game.tactics.mentality = el.dataset.ment;
      saveSoon();
      render();
    }));
  // data-pressing ולא data-press: שני המסכים חלקו את אותו שם, ולכן
  // לחיצה על "להכחיש" בעיתונות גם שינתה את הלחץ הטקטי, ולהפך.
  app.querySelectorAll("[data-pressing]").forEach(el =>
    el.addEventListener("click", () => {
      game.tactics.pressing = el.dataset.pressing;
      saveSoon();
      render();
    }));
...
function resolveChoice(index) {
  const event = game.pendingEvent();
  const title = fillStory(event.title, game);
...
function boot() {
  ...
  // פעימת לב: אירועי מחזור החיים לא נורים כשהדפדפן קורס או כשמערכת
  // ההפעלה הורגת לשונית ברקע, ואז מהלך שנעשה ולא נשמר פשוט נעלם.
  // הבדיקה עצמה זולה, ולכן היא רצה כל כמה שניות ושומרת רק כשזז משהו.
  setInterval(() => {
    if (!game) return;
    const stamp = stateStamp();
    if (stamp && stamp !== lastStamp) saveGame();
  }, HEARTBEAT_MS);
