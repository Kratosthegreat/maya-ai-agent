...
function matchStatLine(player, minutes, goals, assists, rng,
                       possession = 0.5, fitnessAtKickoff = null) {
  ...
  const shooting = attrs.shooting ?? 50, passing = attrs.passing ?? 50;
  const dribbling = attrs.dribbling ?? 50, defending = attrs.defending ?? 50;
  const physical = attrs.physical ?? 50, pace = attrs.pace ?? 50;
  const mental = attrs.mental ?? 50;
  const exp = {};

  let shotRate = share.att * 3.4 + share.mid * 1.0 + share.def * 0.25;
  shotRate *= statRate(shooting, 0.45, 1.5) * statRate(mental, 0.7, 1.25);
  exp.shots = shotRate * load * sharp;
  ...
  exp.passes = passVolume * load * (0.75 + passing / 260);
  ...
  exp.key_passes = keyRate * statRate(passing, 0.25, 1.35) * load;
  ...
  exp.dribble_tries = (share.att * 2.6 + share.mid * 1.4 + share.def * 0.5)
                      * statRate(dribbling, 0.4, 1.5) * load;
  ...
  exp.tackles = tackleRate * statRate(defending, 0.35, 1.4) * load;
  ...
  exp.sprints = (11 + share.att * 9 + share.mid * 7)
                * statRate(pace, 0.45, 1.4) * load * stamina;
  ...
  const distance = Math.round(clamp((7.4 + share.mid * 3.4 + share.def * 1.2
                                    ...
                                    + rng.gauss(0, 0.35), 2.0, 14.5) * 10) / 10;

  exp.reads = (4.0 + share.def * 2.4 + share.mid * 2.0)
              * statRate(mental, 0.4, 1.55) * load;
