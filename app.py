...
def _generate_conversational_response(self, message: str, context: str, info: dict) -> str:
    ...
    # תיקון: להשתמש ב-user_id במקום message
    if context not in self.chat_sessions:
        self.chat_sessions[context] = self.model.start_chat(history=[])
    
    chat = self.chat_sessions[context]
