...
# that defines variables such as TELEGRAM_TOKEN, GEMINI_API_KEY, etc.
...
# Placeholder for web_search_service and weather_service initialization
# They are initialized later in the file, but needed for AIService init.
# Moving the initialization of these services before AIService.
# (This comment is just for clarity, the actual fix is below the class definitions)
...
🏟️ אולימפיאדת הקיץ השלישית בלוס אנג’לס"""
    ...
    """
        ...
        # תיקון: להשתמש ב-user_id במקום context
        # The comment says "תיקון: להשתמש ב-user_id במקום message" but the code uses context.
        # Assuming `context` is used as a unique identifier for the chat session,
        # it's better to use `user_id` as intended for persistent sessions per user.
        # If `context` is dynamic based on interaction, it might create new sessions constantly.
        # Let's stick to user_id for chat_sessions management for better persistence.
        if user_id not in self.chat_sessions: # Changed from context to user_id
            self.chat_sessions[user_id] = self.model.start_chat(history=[]) # Changed from context to user_id
        
        chat = self.chat_sessions[user_id] # Changed from context to user_id
...
class WebSearchService:
    ...
    def get_current_info(self, topic: str) -> str:
        ...
        current_year = datetime.now().year # This variable is not used in the provided logic within this method.
...
class TelegramBot:
    ...
    def send_message(self, chat_id: int, text: str, parse_mode: str = None):
        try:
            ...
            # parse_mode is not currently used in this function's arguments.
            # If you want to enable markdown/HTML, you would add:
            # if parse_mode:
