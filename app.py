"""
Maya Secretary Bot - Fixed Syntax Error
"""
...
# === AI SERVICE ===
class AIService:
    """AI service for generating responses - Natural Hebrew"""
    ...
    def _get_system_instruction(self) -> str:
        """Get system instruction text - Natural Israeli Hebrew"""
        return """
        את מאיה, מזכירה ישראלית דוגרית וחברותית.
        
        כללי תקשורת:
        - תשובות קצרות וישירות (1-2 משפטים מקסימום)
        - עברית טבעית ורהוטה, ללא פורמליות מיותרת
        - אם לא יודעת משהו - תגידי ישר ותציעי לחפש באינטרנט
        - היי דוגרית אבל חמה ומועילה
        - השתמשי באמוג'י בסוף התשובות
        
        דוגמאות לטון נכון:
        "בטח! אני עושה את זה עכשיו 👍"
        "לא יודעת, בואי נחפש באינטרנט 🔍"
        "רגע, אני בודקת לך 🕒"
        """
    ...
    def generate_response(self, user_id: str, message: str, context: str = "") -> str:
        ...
        try:
            ...
            עני בקצרה ובטבעיות, כמו מזכירה ישראלית אמיתית.
            """
            ...
            return "אופס, יש לי תקלה קטנה. תנסה שוב? 😅"
...
class UserService:
    ...
    def get_user_context(self, user_id: str) -> str:
        ...
        context = f"שם: {user.get('first_name', 'חבר')} | זיכרונות: {', '.join(user_memories[-3:]) if user_memories else 'אין'}"
        return context
...
class WeatherService:
    ...
    CITIES = {
        ...
        "ארגנטינה": (-34.6118, -58.3960),  # Buenos Aires
        "בואנוס איירס": (-34.6118, -58.3960)
    }
    ...
    def get_weather(self, city: str = "תל אביב") -> str:
        ...
        try:
            ...
            return f"🌤️ ב{city}: {temp}°C, רוח {windspeed} קמ\"ש"
            
        except Exception as e:
            logger.error(f"Weather error for {city}: {e}")
            return f"לא הצלחתי לקבל מזג אוויר ל{city} 😕"
...
class TelegramBot:
    ...
    def send_message(self, chat_id: int, text: str):
        """Send message to Telegram - Fixed version"""
        try:
    ...
    def process_update(self, update: Dict[str, Any]):
        ...
        try:
            ...
            if security.is_rate_limited(str(user_data_tg.get("id", 0))):
                self.send_message(chat_id, "רגע, יותר מדי בקשות. תן לי דקה 😊")
                return
    ...
    def _handle_command(self, chat_id: int, command: str, user: Dict[str, Any]):
        ...
        if cmd == "/start":
            response = f"היי {user['first_name']}! אני מאיה, המזכירה שלך 🙋‍♀️\nמה נעשה היום?"
        
        elif cmd == "/help":
            response = """🤖 מאיה - המזכירה שלך
...
פשוט תגיד לי מה אתה צריך! 💪"""
            ...
            response = f"📊 יש לי {total_users} חברים ו-{total_conversations} שיחות! 🎉"
            ...
            save_data()
            response = "שכחתי הכל! נתחיל מחדש 🧹"
        
        else:
            response = "לא מכירה את הפקודה הזו 🤔"
    ...
    def _handle_message(self, chat_id: int, text: str, user: Dict[str, Any]):
        ...
        # Time check
            ...
            response = f"🕐 השעה: {now.strftime('%H:%M')} ({now.strftime('%A')})"
