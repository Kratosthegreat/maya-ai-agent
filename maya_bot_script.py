...
class GeminiTracker:
    ...
    def load_usage_data(self):
        if os.path.exists(self.usage_file):
            try:
                ...
                logger.debug(f"📊 Loaded Gemini usage: {self.usage_data.get('daily_requests', 0)} requests today")
            except Exception as e:
    ...
    def _init_usage_data(self):
        ...
        logger.info("📊 Initialized Gemini usage tracking")
    ...
    def can_make_request(self):
        ...
        if self.usage_data['last_reset'] != today:
            ...
            logger.info(f"🔄 Daily reset - Previous: {self.usage_data['daily_requests']} requests")
        ...
        if self.usage_data['daily_requests'] >= self.daily_limit:
            return False, f"הגעת למגבלה היומית ({self.daily_limit} בקשות)"
        
        if len(self.usage_data['minute_requests']) >= self.minute_limit:
            return False, f"יותר מדי בקשות בדקה ({self.minute_limit}). נסה בעוד דקה"
        
        remaining = self.daily_limit - self.usage_data['daily_requests']
        return True, f"יכול לבצע בקשה (נותרו: {remaining})"
    ...
    def record_request(self, tokens_used=0):
        ...
        self.save_usage_data()
        logger.info(f"📊 Request recorded: {self.usage_data['daily_requests']}/{self.daily_limit}, Tokens: {tokens_used}")
...
class AIService:
    ...
    def __init__(self):
        try:
            ...
            self.tracker = GeminiTracker()  # NEW: Add tracker
            logger.info(f"✅ AI Service initialized with model: {config.GEMINI_MODEL}")
            
            # Log initial usage
            stats = self.tracker.get_usage_stats()
            logger.info(f"📊 Today's usage: {stats['daily_requests']}/{stats['daily_limit']} ({stats['percentage_used_today']}%)")
    ...
    def generate_response(self, user_id: str, message: str, context: str = "") -> str:
        ...
        try:
            # NEW: Check if we can make a request
            can_request, status_message = self.tracker.can_make_request()
            if not can_request:
                logger.warning(f"🚫 Gemini limit reached: {status_message}")
    ...
    def get_usage_stats(self):  # NEW method
